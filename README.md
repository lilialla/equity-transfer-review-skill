...
它的核心目标不是做合同润色，而是围绕交易是否可交割建立审查框架：交易类型分流、交易画像、权属核验、出资责任、转让限制、内部批准、监管路径、税务/外汇事项、资料缺口、红旗风险、条款修改建议和交割条件。
...
- 增加交易画像和证据地图。
- 增加交易类型分流，以及权属、出资、限制、程序、监管、税务/外汇、价款和交割维度的红旗分类。
- 增加资料缺口、条款修改、交割条件和 memo 输出模板。
- 增加 evidence-grade 输出纪律，使用 `verified`、`single_source_database`、`platform_found`、`case_file_claim`、`model_inference`、`unverified`、`conflict` 标签区分来源状态。
- 增加复杂场景的官方来源刷新要求。
